...
required <- c("tidyverse", "tidylog", "magrittr", "here", "janitor", "readxl", "digest", "rgbif","countrycode")
...
library(countrycode)    # To generate country codes (e.g. iso3c)
```
...
```{r}
input_data <- read_excel(path = here("data", "raw", "privateData.xlsx")) #edit file name
```
...
```{r}
distribution %>%
  group_by(country, locality, dwc_locality) %>%
  count()
```
...
```{r}
distribution %<>% mutate(country=case_when(
  country == "England" ~ "United Kingdom",
  country == "Northern Ireland" ~ "United Kingdom",
  TRUE ~ as.character(country)
  ))

distribution %<>% mutate(countryCode = countrycode(
  sourcevar = country,
  origin = "country.name",
  destination = c("iso2c"), 
  warn = TRUE,
...
distribution %<>% mutate(dwc_countryCode = countryCode) 
```
...
```{r}
distribution %<>% mutate(dwc_occurrenceStatus = occurrence) # 1 for presence
```
...
```{r}
distribution %<>% mutate(dwc_source = dataSource, 
                         dwc_references = doiUrl)
```

### coordinates and event date
Map values:
```{r}
distribution %<>% mutate(dwc_decimalLatitude = lat, 
                         dwc_decimalLongitude = long,
                         dwc_year = year,
                         dwc_month = month)
```
...
distribution %>% head()
```
## Join taxonomic info to distribution dataset

```{r}
distribution <- left_join(distribution, taxon,
                          by = "taxonID")

distribution %<>%
  mutate(species = sub("(\\w+\\s+\\w+).*", "\\1", scientificName))
```
