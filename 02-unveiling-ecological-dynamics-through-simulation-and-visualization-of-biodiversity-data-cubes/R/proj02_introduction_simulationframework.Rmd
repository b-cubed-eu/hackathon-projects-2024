...
You can download R and RStudio from [here](https://posit.co/download/rstudio-desktop/) adn Git from [here](https://git-scm.com/downloads).
...
Therefore R code to assign observations to grid cells is already available.
...
```{r}
ggplot() +
  geom_sf(data = grid_df, fill = alpha("cornflowerblue", 0.5), linewidth = 1) +
...
The first argument (`observations`) is an sf object with POINT geometry and a `coordinateUncertaintyInMeters` column.
...
Further we have the optional argument `id_col` which is the column name of the column with unique ids for each grid cell. If NULL (the default), a column `"id"` is created were the column numbers represent the unique ids.
And the final argument seed which you can specify for random number generation to make results reproducible. If NULL (the default), no seed is used.
